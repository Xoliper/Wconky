...
Node * XmlParser::getNode(std::initializer_list <std::string> nodesNames){
  Node * curNode = rootNode;
  if(nodesNames.size() == 1 and (*nodesNames.begin()) == "") return curNode;
  std::string nTemp;
...
void XmlParser::prepareRootNode(){
	data.clear();
	if(rootNode != nullptr){
		delete rootNode;
	}
...
Node * XmlParser::addNode(std::string nodeName, std::initializer_list <std::string> path){

	  Node * parentNode = getNode(path);
	  if(parentNode != nullptr){
		  Node * newNode = new Node(nodeName, parentNode);
		  parentNode->nodes.push_back(newNode);
		  return newNode;
	  }
	  return nullptr;
}

Node * XmlParser::addNode(std::string nodeName, Node * parentNode){
	  if(parentNode != nullptr){
		  Node * newNode = new Node(nodeName, parentNode);
		  parentNode->nodes.push_back(newNode);
		  return newNode;
...
void XmlParser::setNodeValue(Node * node, std::string value){
	node->value = value;
}


void XmlParser::setAttribute(Node * node, std::string key, std::string value){
	node->params.insert(std::pair<std::string, std::string>(key, value));
}

void XmlParser::save(std::string path){

	if(rootNode->nodes.size() == 0){
		throw(Exception("XmlParser", "save", "Saving empty tree is not allowed!"));
	}

	std::string output;
	saveXmlHelper("", output, rootNode->nodes.front());

    std::fstream sFile(path.c_str(), std::ios::out | std::ios::trunc );
    if( sFile.good() ){
    	sFile.write( &output[0], output.length() );
    	sFile.flush();
    	sFile.close();
    } else {
		throw(Exception("XmlParser", "save", "Cannot create/open file for writing!"));
    }

}

void XmlParser::saveXmlHelper(std::string spaces, std::string & output, Node * nd){
	int endType = -1;
	if(nd->params.empty() and nd->value.empty()){
		output += spaces+"<"+nd->name+">\n";
		endType = 0;
	} else if(nd->nodes.empty() and nd->value.empty()){
		output += spaces+"<"+nd->name;
		//Print params
		for(auto x = nd->params.begin(); x != nd->params.end(); x++){
			output += " "+x->first+"=\""+x->second+"\"";
		}
		output += " />\n";
		endType = 1;
	} else if(!nd->value.empty()){
		output += spaces+"<"+nd->name;
		//Print params
		for(auto x = nd->params.begin(); x != nd->params.end(); x++){
			output += " "+x->first+"=\""+x->second+"\"";
		}
		output += ">"+nd->value+"</"+nd->name+">\n";
		endType = 1;
	} else if(nd->value.empty()){
		output += spaces+"<"+nd->name;
		//Print params
	...
	for( auto it = nd->nodes.begin(); it != nd->nodes.end(); it++){
		saveXmlHelper(spaces+"   ", output, (*it));
	}
...
void XmlParser::printXml(Node * nd){
	std::cout<<std::endl;
	if(nd == nullptr){
		printXmlHelper("  ", rootNode);
	} else {
		printXmlHelper("  ", nd);
	}
}

void XmlParser::printXmlHelper(std::string spaces, Node * nd){
	int endType = -1;
	if(nd->params.empty() and nd->value.empty()){
		std::cout<<spaces<<"<"<<nd->name<<">\n";
		endType = 0;
